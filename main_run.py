...
if len(sys.argv) >= 4:
    ...
    hyperparams_name = (
        "vanilla_one_store"  # Vanilla neural network for one store setting
    )
else:
...
config_hyperparams_file = (
    f"config_files/policies_and_hyperparams/{hyperparams_name}.yml"
)
...
trainer_params, optimizer_params, nn_params = [
    config_hyperparams[key] for key in hyperparams_keys
]
...
train_loader = DataLoader(
    train_dataset, batch_size=params_by_dataset["train"]["batch_size"], shuffle=True
)
...
if trainer_params["load_previous_model"]:
    ...
    model, optimizer = trainer.load_model(
        model, optimizer, trainer_params["load_model_path"]
    )
